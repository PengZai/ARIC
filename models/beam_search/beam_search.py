...
class BeamSearch(object):
    ...
    def apply(self, visual: utils.TensorOrSequence, prev_ids=None, out_size=1, return_probs=False, **kwargs):
        ...
        if prev_ids != None:
            ...
            outputs = []
            log_prob, past = self.model.forward(visual, prev_ids)
            self.selected_words = prev_ids[:, -1].unsqueeze(dim=-1)
        ...
        with self.model.statefulness(self.b_s):
            for t in range(prev_ids_seq_len, self.max_len):
        ...
        outputs = torch.gather(outputs, 1, sort_idxs.expand(self.b_s, self.beam_size, self.max_len-prev_ids_seq_len))
        log_probs = torch.cat(self.log_probs, -1)
        log_probs = torch.gather(log_probs, 1, sort_idxs.expand(self.b_s, self.beam_size, self.max_len-prev_ids_seq_len))
        if return_probs:
        ...
        # if prev_ids != None:
        #     outputs = torch.cat([prev_ids, outputs], dim=1)
    ...
    def iter(self, t: int, visual: utils.TensorOrSequence,  outputs,  past, return_probs, **kwargs):
        cur_beam_size = 1 if t == 0 or len(outputs) == 0 else self.beam_size
