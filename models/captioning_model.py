...
class CaptioningModel(Module):
    ...
    def beam_search(self, visual: utils.TensorOrSequence, max_len: int, eos_idx: int, beam_size: int, prev_ids=None, out_size=1,
                    return_probs=False, **kwargs):
        bs = BeamSearch(self, max_len, eos_idx, beam_size)
        return bs.apply(visual, prev_ids, out_size, return_probs, **kwargs)
